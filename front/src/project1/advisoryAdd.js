  ...
  const addAdvisory = async () => {
    try {
      let response = await fetch("http://localhost:5000/graphql", {
        ...
        body: JSON.stringify({
          query: `mutation {postadvisory(travellername: "${state.travellerName}", name: "${state.selectedcountry}") {travellername,name,date, text}}`
        })
      });
      let json = await response.json();
      props.dataFromChild(`added advisory on ${json.data.postadvisory.date}`);
      setState({
        travellerName: "",
        selectedcountry: "",
        reset: true
